...
func (b Battle) Log(fromNftEdition uint, toNftEdition uint) BattleLog {
	// Replace the log placeholders with actual NFT edition numbers
	fromFlunkStr := fmt.Sprintf("Flunk #%d", fromNftEdition)
	toFlunkStr := fmt.Sprintf("Flunk #%d", toNftEdition)

	actionMsg := strings.Replace(b.Action, "{Flunk_#1}", fromFlunkStr, -1)
	actionMsg = strings.Replace(actionMsg, "{Flunk_#2}", toFlunkStr, -1)

	actionOutcomeMsg := strings.Replace(b.Outcome.displayMsg, "{Flunk_#1}", fromFlunkStr, -1)
	actionOutcomeMsg = strings.Replace(actionOutcomeMsg, "{Flunk_#2}", toFlunkStr, -1)

	var battleOutcomeMsg string
	if b.Winner == 0 {
		battleOutcomeMsg = fmt.Sprintf("%s won the battle!", fromFlunkStr)
	} else {
		battleOutcomeMsg = fmt.Sprintf("%s won the battle!", toFlunkStr)
	}

	return BattleLog{
		Action:        actionMsg,
		ActionOutcome: actionOutcomeMsg,
		BattleOutcome: battleOutcomeMsg,
	}
}
	...
	"Jock": {
		{
			...
			outcomeOptions: []WeaponOutcomeOption{
				...
				{
					displayMsg:    "That should not be allowed in school, {Flunk_#1} has turned green after taking the protein shake and SMASHES {Flunk_#2} to smithereens.",
					challengerWon: true,
				},
				{
